...
function shouldProxy(value: any): boolean {
    if (typeof value !== 'object' || value === null) return false;
    if (STATE_SYMBOL in value) return false; // already proxied
    return (
        Array.isArray(value) ||
        isPlainObject(value) ||
...
export function proxy<T>(value: T, _parentRoot?: State): T {
    ...
    if (target instanceof Map) {
        result = proxyMap(target, _parentRoot);
    } else if (target instanceof Set) {
        result = proxySet(target, _parentRoot);
    } else if (target instanceof Date) {
        result = proxyDate(target, _parentRoot);
    } else {
        result = proxyObject(target, _parentRoot);
    }

    proxyCache.set(target, result);
    return result;
}
