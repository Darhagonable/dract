 ...
 *
 * Compares dartsxToTsx(input.tsx) against output.tsx in the dartsx-to-tsx/ folder.
 * Run UPDATE_SNAPSHOTS=true to regenerate output.tsx from current output.
 */
import { describe, it, expect } from 'vitest';
import { readFileSync, writeFileSync } from 'fs';
import { join } from 'path';
import { dartsxToTsx } from '../src/dartsx-to-tsx.js';
...
describe('dartsx-to-tsx', () => {
	it('snapshot', () => {
		...
		const { code } = dartsxToTsx(input.trim());
