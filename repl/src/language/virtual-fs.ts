// The language worker's virtual file system: TypeScript's default libs and
// the dartsx runtime's .d.ts files, fetched once at boot from
// /dartsx-lang-fs.json (served by the languageTypesFs() plugin in
// vite.config.ts). Everything is mounted under /node_modules/** — the root
// @volar/typescript falls back to when resolving libs on the web — so
// project files under /project/ resolve 'dartsx' by walking up to it.
...
export async function loadVirtualFs(): Promise<void> {
	...
	for (const [path, text] of Object.entries(payload)) addFile(path, text);
	loaded = true;
}
...
export const virtualFs = {
	...
	readDirectory(path: string): [string, FileType][] {
		...
		return [...names].map((name) =>
			name.endsWith('/') ? [name.slice(0, -1), 2 as FileType] : [name, 1 as FileType],
		);
	},
};
