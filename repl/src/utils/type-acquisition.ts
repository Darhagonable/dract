...
export function createTypeAcquisition(fsMap: Map<string, string>): TypeAcquisition {
	...
	const pickJsxImportSource = (packages: Map<string, PackageFiles>) => {
		// Our jsx-runtime always comes from the BUNDLED dartsx types, which never
		// pass through acquisition — so consult both sets.
		const manifest =
			packages.get('dartsx')?.files.get('/package.json') ??
			fsMap.get(`${MOUNT_PREFIX}dartsx/package.json`);
		if (!manifest) return undefined;
		try {
			return JSON.parse(manifest).exports?.['./jsx-runtime'] ? 'dartsx' : undefined;
		} catch { }
		return undefined;
	};
